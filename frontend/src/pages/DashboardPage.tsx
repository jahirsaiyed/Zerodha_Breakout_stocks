...
export function DashboardPage() {
  ...
  const pnlStr = (totalPnl >= 0 ? '+' : '') + '₹' + totalPnl.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard label="Total Realised P&L" value={pnlStr} color={pnlColor} sub={`${closed.length} closed trades`} />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 text-left">
                {['Symbol','Qty','Avg Entry','SL','Target','P&L','Status'].map(h => (
              ...
              {active.slice(0, 8).map(pos => (
                <tr key={pos.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50">
                  ...
                  <td className="px-5 py-3">
                    <span className={pos.realisedPnl != null
                      ? (pos.realisedPnl >= 0 ? 'text-emerald-600' : 'text-red-600')
                      : 'text-gray-400'}>
                      {pos.realisedPnl != null ? (pos.realisedPnl >= 0 ? '+' : '') + pos.realisedPnl.toFixed(2) : '—'}
                    </span>
                  </td>
                  <td className="px-5 py-3">
